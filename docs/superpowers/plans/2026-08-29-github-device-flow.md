...
| `apps/desktop/src-tauri/src/github_app_config.rs`                | Public Client ID resource loader and installed-app validation.                               |
...
| `apps/desktop/src-tauri/tests/fixtures/github-device-start.json` | Shared non-secret Device Flow response contract fixture.                                     |
...
- Create: `apps/desktop/src-tauri/src/github_app_config.rs`
...
#[test]
fn connection_metadata_contains_no_credential_fields() {
    let value = ConnectionMetadata::connected("octocat", "2026-08-29T00:00:00Z", "2026-08-29T08:00:00Z", "v1");
...
Add `github_app_config.rs` with `GithubAppConfig::load(app)`. In production it
reads only the signed `github-app.json` Tauri resource; test/development accepts
only `RESEARCHLEDGER_GITHUB_CLIENT_ID`. Parse exactly `clientId` and
`configurationVersion`, reject empty/control-character values, and return
`AppMisconfigured` without an HTTP call. Add a fixture test for missing resource,
malformed JSON, valid public ID, and rejection of `clientSecret` or token keys.

Persist `ConnectionMetadata { login, authorized_at, access_expires_at,
configuration_version, state }` atomically under the app config directory. Its
states are `connected` and `disconnecting`; it has no credential fields.
...
```bash
git add apps/desktop/src-tauri/Cargo.toml apps/desktop/src-tauri/Cargo.lock apps/desktop/src-tauri/src/github_credentials.rs apps/desktop/src-tauri/src/github_app_config.rs apps/desktop/src-tauri/src/lib.rs
...
- Modify: `apps/desktop/src-tauri/src/lib.rs`
- Create: `apps/desktop/src-tauri/tests/fixtures/github-device-start.json`
...
Create `tests/fixtures/github-device-start.json`:
...
```json
{
  ...
  "userCode": "ABCD-EFGH",
  "expiresAt": "2026-08-29T00:15:00Z",
  "pollIntervalSeconds": 5
}
```

Deserialize the fixture into `DeviceStartView` and reserialize it exactly. This
is the shared command/UI contract; it contains no `device_code` or token.
...
#[test]
fn checkpoints_are_scoped_to_the_selected_vault_database() {
    let first = temp_root();
    let second = temp_root();
    let first_db = open(&initialize(&first).unwrap()).unwrap();
    create_github_import_run(&first_db, "run-1", "octocat", "v1", "2026-08-29T00:00:00Z").unwrap();
    let second_db = open(&initialize(&second).unwrap()).unwrap();
    assert_eq!(github_import_run(&second_db, "run-1").unwrap(), None);
}
```
...
`last_repository`, `pending_repository`, `pending_phase`, `started_at`,
`updated_at`, and `error_category`. Add `github_import_items` keyed by run id
and repository identity with `outcome`, `reason_category`, `retry_count`,
`retry_after`, and `updated_at`. Do not add token, authorization-code, README,
or raw response columns. Add
`GithubImportRun`, `create_github_import_run`, `checkpoint_github_import_run`,
`github_import_run`, `github_import_items`, `record_github_import_item`, and
`finish_github_import_run` functions. State in a storage test that these rows
live only in the selected vault's `.researchledger.db`, never in a shared path.
...
NotReadableOrUnavailable, PermissionDenied, Unauthorized, InvalidPayload}`. Read
`Retry-After`, `x-ratelimit-remaining`, and
`x-ratelimit-reset` before categorizing `403`; only zero remaining is a primary
rate-limit outcome. Treat `403` or `429` with `Retry-After`, zero remaining, or
the documented rate-limit response category as `RetryAfter`; otherwise preserve
`PermissionDenied`. A README `404` is `Unavailable` only for a public repository;
a private repository `404` is `NotReadableOrUnavailable`.
...
becoming `Unauthorized`, never `RetryAfter`. Add `403` and `429` secondary-limit
fixtures with nonzero remaining plus `Retry-After`; both become `RetryAfter`.
Add separate public-404 and private-404 fixtures with their required results.
Record every fixture outcome in `github_import_items`.

Add a pagination-mutation fixture: page one returns repositories `r001..r100`,
then a newly starred `r000` shifts the next response. Request stars with
`sort=created&direction=desc`, replay one completed page on resume, and dedupe
by `github:<owner>/<repo>`. Assert `r001..r100` and `r000` are all present once
after the run reaches a response without a `Link: rel="next"` value. Add a
removal variant that proves an already recorded repository is retained and no
unseen repository is skipped by the one-page overlap.
...
`import_github_starred`, `github_import_status`, and `github_import_cancel`.
Register all eight in `generate_handler!`. `GithubImportState` stores only active
run cancellation flags keyed by run id; it is checked between repository units.
...
error returns a fixed category and leaves the connection unchanged. On relaunch,
`github_connection_status` reconciles the Keychain item with `ConnectionMetadata`:
both valid returns `Connected`, either missing/expired returns
`ActionRequired(Reconnect)`, and an interrupted disconnect returns
`ActionRequired(Cleanup)` without claiming a connected account.
...
`gh` path until the native write succeeds. `github_import_cancel` records a
`cancelled` run after the current item transaction; it does not interrupt a
document write and preserves `pending_repository`/`pending_phase` for an
explicit resume.
...
is unchanged before native success; an HTTP `401` requires reconnection; a
README 5xx after metadata write retries then pauses with the item persisted; a
cancelled run resumes its pending README; connection metadata restores a safe
state after relaunch; and a fake token never appears in serialized status or
error strings.
...
it("shows a device code but never a GitHub token field", async () => {
  ...
  expect(invokeMock).toHaveBeenCalledWith("github_device_poll", {
    sessionId: "session-opaque-1",
  });
  expect(
...
an explicit Advanced disclosure. Use the exact shared DeviceStartView fixture;
retain `sessionId` only to poll/cancel the pending flow. During import, poll
`github_import_status` for persisted item outcomes and send `github_import_cancel`
with the displayed `runId`; React unmount must not be the cancellation mechanism.
...
disconnect returning to disconnected. Cover repository-specific retry affordance,
cancel after a completed item, and `ActionRequired(Cleanup)`. Assert no call
contains a property named `token`, `accessToken`, or `refreshToken`.
...
Specify the configuration contract: development/tests may set
`RESEARCHLEDGER_GITHUB_CLIENT_ID`; the release pipeline writes the public-only
`github-app.json` Tauri resource; installed apps read that signed resource and
reject missing, malformed, or secret-bearing configuration. Add installed-app
smoke cases for a missing resource and a valid public Client ID before live
authorization.
