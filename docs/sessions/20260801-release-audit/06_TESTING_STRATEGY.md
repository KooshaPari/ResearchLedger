...
- Retrieval quality: `cross_encoder_contract.json` supplies candidate text, a local `/v1/rerank`
  response, and expected order. Unit tests prove strict response validation, loopback-only
  endpoint policy, and rank application. An operator-installed model smoke is a separate gate.
